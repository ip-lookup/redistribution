...
do
  ...
  curl -s -L "https://download.maxmind.com/app/geoip_download?edition_id=${edition}&license_key=${MAXMIND_LICENSE_KEY}&suffix=zip.sha256" -o ${edition}.zip.remote.sha256
  ...
  then
	...
	cd ..
	./scripts/clean-git-objects.sh ${edition}
	stat=$?
	if [ $stat -ne 0 ]
	then
  ...
  curl -s -L "https://download.maxmind.com/app/geoip_download?edition_id=${edition}&license_key=${MAXMIND_LICENSE_KEY}&suffix=zip" -o ${edition}.zip
  ...
  then
	...
	cd ..
	./scripts/commit-database.sh ${edition}
	stat=$?
	if [ $stat -ne 0 ]
	then
